...
app.get("/users/get_byOrder",
query("sort").notEmpty().withMessage("sort query parameter should not be empty")
.isLength({min:3,max:3}).isString().withMessage("sort parameter value should be string")
,(req,res)=>{
...
app.post("/queryCreate",[
query("uid").toInt().isNumeric().withMessage("user id should be number"),
...
query("age").notEmpty().withMessage("Age should not be empty").isLength({max:3})
...
app.post("/createUser",[
    ...
    body("age").notEmpty().withMessage("Age should not be empty").isLength({max:3})
...
app.put("/update/:id",[
    param("id").toInt().isNumeric().withMessage("user id should be number").
    custom(value => {
        const length = value.toString().length;
        if (length<3) { // Validate length (1-3 digits)
            throw new Error('User ID must be a valid length (1-3 digits).');
        }
        return true; // Valid
    })

],fetchIndexPar,(req,res)=>{
